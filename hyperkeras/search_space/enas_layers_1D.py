...
class FilterAlignment(kl.Layer):
    def __init__(self, name_prefix, filters, data_format, **kwargs):
        ...
        self.conv1d = kl.Conv1D(filters, 1, padding='same', name=f'{name_prefix}_filters_aligning_conv1d_',
...
class FactorizedReduction_K(kl.Layer):
    def __init__(self, name_prefix, filters, data_format, **kwargs):
        ...
        self.x0_avgpool = kl.AveragePooling1D(
            pool_size=1,
            strides=2,
            padding="valid",
            data_format=self.data_format,
            name=f'{name_prefix}avepool1d_a_')
        self.x0_conv = kl.Conv1D(
            filters=filters,
            kernel_size=1,
            strides=1,
            padding="valid",
            data_format=self.data_format,
            name=f"{name_prefix}conv1d_a_")
        self.x1_zeropad = kl.ZeroPadding1D(
            padding=(0, 1),
#             data_format=self.data_format,
            name=f"{name_prefix}zeropad1d_b_")
        self.x1_cropping = kl.Cropping1D(
            cropping=(1, 0),
#             data_format=self.data_format,
            name=f"{name_prefix}crop1d_b_")
        self.x1_avgpool = kl.AveragePooling1D(
            pool_size=1,
            strides=2,
            padding="valid",
            data_format=self.data_format,
            name=f"{name_prefix}avepool1d_b_")
        self.x1_conv = kl.Conv1D(
            filters=filters,
            kernel_size=1,
            strides=1,
            padding="valid",
            data_format=self.data_format,
            name=f"{name_prefix}conv1d_b_")
...
class CalibrateSize(EnasHyperLayer):
    ...
    def get_channels(self, x):
        if self.data_format in {None, 'channels_last'}:
            return x.get_shape().as_list()[2] #because it's 1D using [2] instead of [3]
        else:
