...
from .predefs import HEADER_SIZE, NUM_TYPES, TYPE_MAP, MASKED_VAL



class DataGen2(tensorflow.keras.utils.Sequence):
    """
    Generator class for feeding large data-set of giant PCAPs parsed
    and pre-processed into multiple MAT files to the model
    """
    
    def __init__(self, files, idxfilter=lambda x: True, \
                 batchsize=16, weights=None, sparse=True, \
                 truncate=0, multi_input=True, legacy=0):
        """
    ...
    def mat_readone(self, f, sparse=True, verbose=False):
        ...
        loaded_label = TYPE_MAP[int(loaded_label)]
        ...
        if sparse:
            ...
            out_y = tensorflow.keras.utils.to_categorical(loaded_label, num_classes=NUM_TYPES)
