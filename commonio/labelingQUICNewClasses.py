...
def read_domain_list_from_dir(name, dirname):
    """
    Addition to QUIC.
    
    This function is an adaptation of read_domain_list function that provides the possibility of change de directory location from the dataset.
    """
    ##
    ...
    for n in name:
        # This functions will open the homemade dataset in priority
        # If no homemade dataset with the required name is found, it will instead open the corresponding UT1 file
        if os.path.isfile(dirname + "/{}".format(n)):
            with open(dirname + "/{}".format(n)) as f:
                lines += [line.rstrip() for line in f]
        else:
            # This path is for the modified ut1 dataset.
            with open(dirname + "/{}/domains".format(n)) as f:
                lines += [line.rstrip() for line in f]
...
class Ensemble:
    ...
    @staticmethod
    def get_QUIC202201service_labeling(cached=True):
        # Creates the ensemble for the QUIC new classes

        NAMES = [
            #"chat", 
            "games", "social", "streaming", "ecommerce", "resources", "web"]#["chat", "download", "games", "mail", "search", "social", "streaming", "web"]
        DIR = "/home/orange/domains/homemade_dataset/domainsQUICTestMar"

        # DISCLAIMER : the web class still has interaction with other classes and should remain last in this list.
        # Changing its position in the declaration of games may impact the resulting ensemble
        ENSEMBLE = [
            #("chat", read_domain_list_from_dir("chat", DIR)),
            ("games", read_domain_list_from_dir("games", DIR)),
            ("social", read_domain_list_from_dir("social", DIR)),
            ("streaming", read_domain_list_from_dir("streaming", DIR)),
            ("ecommerce", read_domain_list_from_dir(["ecommerce", "finance"], DIR)),
            ("resources", read_domain_list_from_dir(["resources","marketing", "malware"], DIR)),
            #("finance", read_domain_list_from_dir("finance", DIR)),
            #("marketing", read_domain_list_from_dir("marketing", DIR)),
                        #("download", read_domain_list_from_dir("download", DIR)),
            #("search", read_domain_list_from_dir("search", DIR)),
            #("mail", read_domain_list_from_dir("mail", DIR)),
            #("data", read_domain_list_from_dir(["data"], DIR)),
            ("web", read_domain_list_from_dir("web", DIR))]
    ...
    @staticmethod
    def get_QUIC202201HEREaplication_labeling(cached=True):
        # Creates the ensemble for the QUIC application new classes
    
    
        NAMES = ["chatDiscord", "resourcesGoogle", "resourcesMgid", "resourcesPbstck", "resourcesSmpush", "resourcesCloudflare", "resourcesJSDelivr", "searchGoogle", "socialFacebook", "webCanva"]
        DIR = "/home/orange/domains/homemade_dataset/domainsQUICTest"

        ENSEMBLE = [
            ("chatDiscord", read_domain_list_from_dir("chatDiscord", DIR)),
            #("resourcesAmazon", read_domain_list_from_dir("marketingAmazon", DIR)),
            ("resourcesGoogle", read_domain_list_from_dir("marketingGoogle", DIR)),
            ("resourcesMgid", read_domain_list_from_dir("marketingMgid", DIR)),
            ("resourcesPbstck", read_domain_list_from_dir("marketingPbstck", DIR)),
            ("resourcesSmpush", read_domain_list_from_dir("marketingSmpush", DIR)),
            #("resourcesTwitter", read_domain_list_from_dir("marketingTwitter", DIR)),
            ("resourcesCloudflare", read_domain_list_from_dir("resourcesCloudflare", DIR)),
            ("resourcesJSDelivr", read_domain_list_from_dir("resourcesJSDelivr", DIR)),
            ("searchGoogle", read_domain_list_from_dir("searchGoogle", DIR)),
            #("searchBing", read_domain_list_from_dir("searchBing", DIR)),
            ("socialFacebook", read_domain_list_from_dir("socialFacebook", DIR)),
            #("socialGithub", read_domain_list_from_dir("socialGithub", DIR)),
            ("webCanva", read_domain_list_from_dir("webCanva", DIR)),
            ]
    ...
    @staticmethod
    def get_QUIC202201aplication_labeling(cached=True):
        # Creates the ensemble for the QUIC application new classes
    
    
        NAMES = ["resourcesGoogle", 
#                  "resourcesMgid", 
                 "resourcesPbstck", "resourcesSmpush", "resourcesCloudflare", "resourcesJSDelivr", "socialFacebook", "webCanva"]
        DIR = "/home/orange/domains/homemade_dataset/domainsQUICTest"
