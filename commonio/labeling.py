...
from .conf import UT_CAPITOL_DIR, HOMEMADE_URLS_DIR
...
def read_domain_list(name):
    ...
    lines = []
    for n in name:
        with open(UT_CAPITOL_DIR + "/{}/domains".format(n)) as f:
...
def read_domain_list_newDownload(name):
    ...
    lines = []
    for n in name:
        if os.path.isfile(HOMEMADE_URLS_DIR + "/domainsNewDownload/{}".format(n)):
            with open(HOMEMADE_URLS_DIR + "/domainsNewDownload/{}".format(n)) as f:
                lines += [line.rstrip() for line in f]
        else:
