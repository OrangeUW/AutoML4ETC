...
class DataGenSeparated(tensorflow.keras.utils.Sequence):
    ...
    def __init__(self, files, label_func, weight_func=None,
        ...
        self.fha_flattened = fha_flattened
    ...
    def __getitem__(self, idx):
        ...
        if self.auxiliary:
            ...
            if self.fha_flattened:
                out_x0 = N.reshape(out_x0, (out_x1.shape[0], self.n_packets * self.header_truncate))
                if self.flow_separate_features:
                    out_x1 = N.reshape(out_x1, (out_x1.shape[0], self.n_flows * 3))
                else:
                    out_x1 = N.reshape(out_x1, (out_x1.shape[0], self.n_flows * 2))
                out_x = N.concatenate([out_x0, out_x1, out_x2], axis=-1)
            elif self.flow_only:
                out_x = [out_x1, out_x2]
