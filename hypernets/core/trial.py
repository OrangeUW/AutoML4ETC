...
class TrialHistory():
    ...
    def improved_reward(self, trial):
        old_best = self.get_best()
        #self.trials.append(trial)
       
        if old_best:
            logger.info(f'{old_best}')
            logger.info(f'{old_best.space_sample}')
            logger.info(f'{old_best.reward}')

            return float(trial.reward) > float(old_best.reward)
        else:
            return True
